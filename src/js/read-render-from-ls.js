...
function renderReadNews() {
  const markup = news
    .map(
      el => `<li class="news__item accordion__news" data-id="${el.news.id}"><div class="news__image-box">
  <img class="news__image" src="${el.news.media[0]['media-metadata'][2].url}" alt="" /><div class="div">
    ...
    <h3 class="news__image-heading">${el.news.section}</h3>
</div>
<h2 class="news__title">${el.news.title}</h2>
<p class="news__text">${el.news.abstract}</p>
<div class="news__lower-box">
  <p class="news__date">${el.news.published_date}</p>
  <a class="news__readmore-link" target="_blank"
  rel="noopener noreferrer" href="${el.news.url}">Read more</a>
</div>
</li>
`
    )
